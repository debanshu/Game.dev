...
-- OPTIMISATION: better to start from 2
-- populate table NUMS with numbers from 1 to n
function createTable(n)
	--write code here

	for i=2,n do NUMS[i]=i end

end


-- OPTIMISATION: better to start from square of nth term (all previous terms removed)
...
-- OPTIMISATION: Oly run uptill square root of max, all others eliminated before
-- runs the Eratosthenes algorithm on the table NUMS
function getPrimes(n)
	--write code here
	local i = 2
	local sqn = math.sqrt(n)
	while i<sqn do
		if NUMS[i] ~= nil then
			deleteNth(i,n)
		end
